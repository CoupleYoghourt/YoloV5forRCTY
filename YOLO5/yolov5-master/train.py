...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='yolov5s.pt', help='initial weights path')           #可以用自己的模型练，改路径就好了
    ...
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs')         #一次把多少个数据打包送到网络当中
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes')
    parser.add_argument('--rect', action='store_true', help='rectangular training')                                         #矩形训练，启用的话会减少一些填充图片的冗余部分
    parser.add_argument('--resume', nargs='?', const=True, default=True, help='resume most recent training')#接着之前的训练，dafault可以给last.pt的路径
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')                                 #不保存每个epoch训练的模型，只保存最后一个epoch的
    parser.add_argument('--notest', action='store_true', help='only test final epoch')                                      #不在每个epoch后测试，只在最后一个epoch后测试，我们需要把他设置成每个epoch都测试
    parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check')                             #锚点/锚框；是否调整候选框
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')                                     #超参数更新
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')                                             #没什么用
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')                     #缓存图片
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')           #对上一轮训练不好的图片加一些权重，作者实际测试的时候效果没什么大改善
    ...
    parser.add_argument('--single-cls', action='store_true', help='train as single-class dataset')                          #训练的数据集是单类别还是多类别的
    parser.add_argument('--adam', action='store_true', help='use torch.optim.Adam() optimizer')                             #优化器选择，默认为False，用随机梯度下降
    parser.add_argument('--sync-bn', action='store_true', help='use SyncBatchNorm, only available in DDP mode')             #跨GPU的BN，有DDP字眼的，一般是多GPU的机器使用的
